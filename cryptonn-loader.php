<?php
/**
 * CryptONN Loader v2.3 — Free, Keyless, License-on-Encoder
 *
...
function _cnn_get_master_key(string $license_id): ?string {
    ...
    if (file_exists($cache_file)) {
        ...
        if ($stat && (time() - $stat['mtime']) < 86400) {
            ...
            if ($raw !== false) {
                ...
                // Trial expiry check: if trial has ended, purge cache and go to API
                // so the API can return TRIAL_EXPIRED rather than serving a stale key.
                $trial_ts = $cached_data['trial_ends_at'] ?? null;
                if ($trial_ts !== null && time() > $trial_ts) {
                    @unlink($cache_file);
                    if (function_exists('apcu_delete')) apcu_delete($apcu_key);
                } else {
                    $candidate = _cnn_decrypt_bundle($cached_data, $delivery_key);
                    if ($candidate !== null) {
                        $apcu_ttl = ($trial_ts !== null)
                            ? min(3600, max(60, $trial_ts - time()))
                            : 3600;
                        $cache[$license_id] = $candidate;
                        if (function_exists('apcu_store')) apcu_store($apcu_key, $candidate, $apcu_ttl);
                        return $candidate;
                    }
                }
            }
    ...
    if ($resp !== false) {
        ...
        // Trial expired: API explicitly told us — purge all caches, do not decode.
        if (is_array($data) && ($data['error']['code'] ?? '') === 'TRIAL_EXPIRED') {
            @unlink($cache_file);
            if (function_exists('apcu_delete')) apcu_delete($apcu_key);
            return null;
        }
        ...
        $bundle       = (is_array($data) && ($data['ok'] ?? false) === true)
            ...
            : null;
        $trial_ends_at = $data['data']['trial_ends_at'] ?? null; // int unix ts or null

        if (is_array($bundle)) {
            $candidate = _cnn_decrypt_bundle($bundle, $delivery_key);
            if ($candidate !== null) {
                // Persist bundle + trial metadata so cache can self-expire for trial keys
                $cache_payload = array_merge($bundle, ['trial_ends_at' => $trial_ends_at]);
                @file_put_contents($cache_file, json_encode($cache_payload), LOCK_EX);
                $apcu_ttl = ($trial_ends_at !== null)
                    ? min(3600, max(60, $trial_ends_at - time()))
                    : 3600;
                if (function_exists('apcu_store')) apcu_store($apcu_key, $candidate, $apcu_ttl);
                $cache[$license_id] = $candidate;
                return $candidate;
    ...
    // Trial lisanslar için grace period uygulanmaz: API'ye ulaşılamasa bile
    // süresi dolmuş trial key'i sunmak kabul edilemez.
    if (file_exists($cache_file)) {
        $stat = @stat($cache_file);
